class Player extends Entity {
  ...
  movement() {
    ...
    if (mouseIsPressed) {
      ...
      if (this.position.dist(mouseVector) > UNIT_SIZE / 5) {
  ...
  static createPlayer(manager) {
    ...
    car.drivingSFX = loadSound("./assets/audio/moto_correndo.wav");
    car.spinSFX = loadSound("./assets/audio/moto_derrapando.wav");
    ...
    car.addState("spin", (c) => {
      ...
      if (c.manager.hasEvent(Player.Events.FALL)) {
        spin.apply(c);
        c.position.y -= c.manager.entities.get("road0").scrollSpeed * 2;
    ...
    car.addState("default", (c) => {
      ...
      push();
      textSize(UNIT_SIZE / 2);
      textAlign(CENTER);
      stroke(0);
      strokeWeight(4);
      fill(255);
      text(c.buracosDesviados, 0, -height / 2 + UNIT_SIZE / 2);
      pop();
      ...
      if (c.collision.isColliding) {
        c.manager.addEvent(Player.Events.HIT, 3);
      }
      c.entityDraw();
      ...
      if (c.manager.hasEvent(Player.Events.HIT)) {
        if (c.manager.getEventDuration(Player.Events.HIT) === 2) {
          c.health--;
